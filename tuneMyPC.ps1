...
function step_by_step_tune{
    ...
    ******************************************************
    #>
    $user_update = Read-Host "Would you like to install windows update module? 
    y|n: "
    ...
    if($user_update.ToLower() -eq 'y'){
        ...
        Update-MpSignature
        Write-Host 'Windows update Module Installed '$date -ForegroundColor 
    Yellow 
    }else{
    ...
    #retrieves windows updates and installs them
    ...
    #scans file system defrags and trims C drive(c is most likely the boot sector)
        ...
        Write-Host 'FileSystem scan complete' -ForegroundColor white 
    ...
    $get_secure = Read-Host "Would you like to perform quick security scan? 
    y|n: "
